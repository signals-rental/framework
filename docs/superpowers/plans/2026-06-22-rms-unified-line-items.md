...
## PHASE 2 — Events & actions (OUTLINE — detail at phase start)

> Detail these into bite-sized TDD tasks when P1 is merged-to-branch and green, using the same structure as P1. Each carries its own tests; whole-suite green is still deferred to P6.
...
- **T2.2 Extend `ItemAdded`** — payload `item_type`, `path`, `revenue_group_id`, `itemable_type`/`itemable_id`; `apply()` seeds them; `handle()` writes row then prices+syncs demand only when `item_type->isPriceable()`/`->generatesDemand()` AND product-backed. **Tests:** group add writes a row with zero price/no demand; product add unchanged; replay reproduces `path`/`item_type`.
