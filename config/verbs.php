...
return [
   ...
   */
    ...
    */
    ...
    |
    | Signals disables auto-commit so that the ONLY commit path is the
    | CommitsVerbsEvents concern, which wraps fire() + Verbs::commit() in a
    | single DB transaction. Auto-commit at request/command/job boundaries
    | could otherwise persist an event row without running its projection
    | atomically, orphaning the event store (see App\Concerns\CommitsVerbsEvents).
    */
    'autocommit' => false,
];
