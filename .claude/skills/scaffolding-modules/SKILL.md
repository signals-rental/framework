...
2. **Actions + DTOs + events** — one invocable action per operation that occurs (no inline mutations anywhere), `Gate::authorize` inside; **every mutation fires `AuditableEvent` and dispatches its webhook event** (create/update/archive/restore/delete/merge + domain verbs). Each `AuditableEvent` must produce an `action_logs` row surfaced in `/admin/settings/action-log` — assert this in tests. Spatie Data DTOs only — never Form Requests or API Resources.
   - **Webhook dispatch placement:** call `WebhookService::dispatch()` AFTER the `DB::transaction` closure commits, never inside it — every queue connection is `after_commit: false`, so an in-transaction dispatch delivers a webhook even when the transaction later rolls back. `MergeMember`/`AnonymiseMember` model the correct ordering; `MergeProduct` still dispatches inside the transaction (known defect, todo #137) — do not copy it.
...
| "I'll dispatch the webhook inside the transaction for atomicity" | Queues are `after_commit: false` — an in-transaction dispatch delivers webhooks for rolled-back operations. Dispatch AFTER commit. `MergeProduct` has this defect; don't copy it. |
