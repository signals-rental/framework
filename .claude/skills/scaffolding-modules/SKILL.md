...
12. **Tests + quality gate** — 90% line coverage target: actions (direct invocation + `Event::fake`), API (CRUD, abilities, Ransack `_eq`-family only on SQLite, includes, custom-field shape, validation), policy matrix per role, model (casts/scopes/schema), Livewire pages, seeder meta-tests. Then the mandatory gate: tests → `vendor/bin/pint --dirty --format agent` → `vendor/bin/phpstan analyse` → pr-review agents + `/codex:adversarial-review` (cross-model review via the Codex plugin; skip gracefully if not installed) → fix and repeat. Ask before committing.
