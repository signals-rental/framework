...
@if($item->trashed())
    <button
        x-on:click="open = false; $wire.$parent.restoreOpportunity({{ $item->id }})"
...
@else
    <button
        x-on:click="open = false; $dispatch('open-modal', 'archive-opportunity-{{ $item->id }}')"
        class="s-dropdown-item"
    ...
    <x-signals.modal name="archive-opportunity-{{ $item->id }}" title="Archive Opportunity" size="sm">
        <p>Are you sure you want to archive <strong>{{ $item->subject }}</strong>? The opportunity can be restored later.</p>

        <x-slot:footer>
            <button class="s-btn s-btn-sm" type="button" x-on:click="$dispatch('close-modal', 'archive-opportunity-{{ $item->id }}')">Cancel</button>
            <button class="s-btn s-btn-sm s-btn-danger" type="button" wire:click="$parent.archiveOpportunity({{ $item->id }})" x-on:click="$dispatch('close-modal', 'archive-opportunity-{{ $item->id }}')">Archive</button>
        </x-slot:footer>
    </x-signals.modal>
@endif
