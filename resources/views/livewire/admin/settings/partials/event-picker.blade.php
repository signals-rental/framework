@php
    /** @var string $model — the Livewire array property holding the selected events */
    /** @var array<string, array{label: string, events: array<int, string>}> $groups */
    $allKeys = collect($groups)->flatMap(fn ($group) => $group['events'])->values()->all();
@endphp

<div>
    <div class="mb-2 flex items-center justify-between">
        <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Events</label>
            ...
            <div class="rounded-md border border-zinc-200 dark:border-zinc-700"
                ...
                <div x-show="open" class="border-t border-zinc-100 dark:border-zinc-800 px-3 py-2 space-y-2">
                    @foreach($group['events'] as $event)
                        <label class="flex items-center gap-2 cursor-pointer" wire:key="{{ $model }}-{{ $event }}"
                               x-data="{ checked: ($wire.{{ $model }} ?? []).includes('{{ $event }}') }"
                               x-init="$watch('$wire.{{ $model }}', value => checked = (value ?? []).includes('{{ $event }}'))">
                            <input type="checkbox" wire:model="{{ $model }}" value="{{ $event }}" class="hidden" x-on:change="checked = $el.checked" />
                            <x-signals.checkbox x-bind:class="checked && 'checked'" />
                            <span class="text-sm">{{ \Illuminate\Support\Str::headline(\Illuminate\Support\Str::after($event, '.')) }}</span>
                            <span class="text-xs text-zinc-400 font-mono">{{ $event }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
