<a href="{{ route('products.show', $item) }}" wire:navigate class="s-dropdown-item">
...
<a href="{{ route('products.edit', $item) }}" wire:navigate class="s-dropdown-item" style="text-decoration: none;">
...
@if($item->trashed())
    <button
        x-on:click="open = false; $wire.$parent.restoreProduct({{ $item->id }})"
...
@else
    <button
        x-on:click="open = false; $dispatch('open-modal', 'archive-product-{{ $item->id }}')"
        class="s-dropdown-item"
    ...
    <x-signals.modal name="archive-product-{{ $item->id }}" title="Archive Product" size="sm">
        <p>Are you sure you want to archive <strong>{{ $item->name }}</strong>? The product can be restored later.</p>

        <x-slot:footer>
            <button class="s-btn s-btn-sm" type="button" x-on:click="$dispatch('close-modal', 'archive-product-{{ $item->id }}')">Cancel</button>
            <button class="s-btn s-btn-sm s-btn-danger" type="button" wire:click="$parent.archiveProduct({{ $item->id }})" x-on:click="$dispatch('close-modal', 'archive-product-{{ $item->id }}')">Archive</button>
        </x-slot:footer>
    </x-signals.modal>
@endif
